...
def generate_cover_image(slug, theme):
    """生成封面图"""
    prompt = f"Deep sea dark tech style, dark blue background #0A1420, cyan glow effects, {theme}, 16:9 aspect ratio, no text, minimalist modern design, soft gradients, professional quality"
    ...
    try:
        resp = requests.post(IMAGE_URL, headers=headers, json=data, timeout=120)
        ...
        if "data" in result and "image_urls" in result["data"] and len(result["data"]["image_urls"]) > 0:
            ...
            output_path = COVERS_DIR / f"{slug}.jpg"
            with open(output_path, "wb") as f:
                f.write(img_resp.content)
            return True, f"封面图已保存: {output_path}"
        else:
            return False, f"API返回异常: {result}"
    except Exception as e:
        return False, f"生成封面图失败: {e}"
...
def generate_podcast_audio(slug, title, summary):
    ...
    try:
        ...
        if "data" in result and "data" in result["data"] and "audio" in result["data"]["data"]:
            audio_hex = result["data"]["data"]["audio"]
            audio_bytes = binascii.unhexlify(audio_hex)
            
            output_path = AUDIO_DIR / f"{slug}.mp3"
            with open(output_path, "wb") as f:
                f.write(audio_bytes)
            return True, f"音频已保存: {output_path}"
        else:
            return False, f"API返回异常: {result}"
    except Exception as e:
        return False, f"生成音频失败: {e}"

def update_frontmatter(slug):
    """更新文章的frontmatter"""
    ...
    try:
        ...
        # 检查是否已有coverImage和audioUrl
        has_cover = "coverImage:" in content
        has_audio = "audioUrl:" in content
        
        cover_line = f'coverImage: "/covers/{slug}.jpg"'
        audio_line = f'audioUrl: "/audio/{slug}.mp3"'
        
        if has_cover and has_audio:
            # 替换现有值
            import re
            content = re.sub(r'coverImage:\s*"[^"]*"', cover_line, content)
            content = re.sub(r'audioUrl:\s*"[^"]*"', audio_line, content)
        elif has_cover:
            # 替换coverImage，添加audioUrl
            import re
            content = re.sub(r'coverImage:\s*"[^"]*"', cover_line, content)
            # 在coverImage行后添加audioUrl
            content = re.sub(rf'({re.escape(cover_line)})', rf'\1\n{audio_line}', content)
        else:
            # 在frontmatter中添加两者
            # 找到frontmatter的结束位置
            fm_end = content.find("---", 3)
            if fm_end > 0:
                new_lines = f"{cover_line}\n{audio_line}\n"
                content = content[:fm_end] + new_lines + content[fm_end:]
        
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(content)
        return True, f"Frontmatter已更新: {file_path}"
    except Exception as e:
        return False, f"更新frontmatter失败: {e}"
...
def process_article(slug, info):
    ...
    # 生成封面图
    success, msg = generate_cover_image(slug, info["cover_theme"])
    results.append(f"[封面] {msg}")
    
    # 生成音频
    success, msg = generate_podcast_audio(slug, info["title"], info["summary"])
    results.append(f"[音频] {msg}")
    
    # 更新frontmatter
    success, msg = update_frontmatter(slug)
    results.append(f"[Frontmatter] {msg}")
    
    return "\n".join(results)

def report_progress(current, total, slug):
    """汇报进度"""
    progress_file = "/tmp/swarm_blog_progress.txt"
    with open(progress_file, "a") as f:
        f.write(f"[{current}/{total}] 完成: {slug}\n")
    print(f"进度: {current}/{total} - {slug}")

if __name__ == "__main__":
    import sys
    
    if len(sys.argv) < 2:
        print("用法: python batch_generate.py <slug>")
        sys.exit(1)
