...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate postSlug to prevent path/key injection into Redis keys
    if (typeof postSlug !== 'string' || !/^[a-zA-Z0-9\-_\u4e00-\u9fa5]{1,200}$/.test(postSlug)) {
      return NextResponse.json({ error: 'Invalid postSlug format' }, { status: 400 });
    }
